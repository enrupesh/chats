...
**How to apply:** If `node_modules` is absent or a declared package cannot be resolved, run the workspace's frozen lockfile install first. Then run the shared package build before client/server checks. Do not replace the workspace package with a registry dependency or restructure the monorepo.