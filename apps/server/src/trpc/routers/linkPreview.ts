...
// Keep this separate from the raw URL cache key so a preview generated by an
// older server version cannot survive after the waitlist card is corrected.
const WAITLIST_CACHE_KEY = "__veilchat_waitlist_preview_v2__";

function isVeilChatWaitlistUrl(url: URL): boolean {
  const host = url.hostname.toLowerCase();
  const pathname = url.pathname.replace(/\/+$/, "") || "/";
  return (
    (host === "www.veilchat.me" || host === "veilchat.me") &&
    pathname === "/waitlist"
  );
}
...
async function fetchPreview(rawUrl: string): Promise<LinkPreview> {
  ...
  const isWaitlist = isVeilChatWaitlistUrl(url);
  if (isWaitlist) {
...
export const linkPreviewRouter = router({
  ...
  fetch: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const cacheKey = isVeilChatWaitlistUrl(parsedUrl)
        ? WAITLIST_CACHE_KEY
        : input.url;
