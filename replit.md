...
VeilChat also ships as a native Android app using **Capacitor 8** (package: `me.veilchat.app`).
The Android project lives at `apps/client/android/` alongside the web client — same codebase, single git repo.

### Key files added
| File | Purpose |
|------|---------|
| `apps/client/capacitor.config.ts` | Capacitor config (appId, plugins, status bar, splash) |
| `apps/client/android/` | Full Gradle / Android Studio project (generated by `cap add android`) |
| `apps/client/src/lib/capacitor.ts` | Platform detection (`isAndroid()`, `isNative()`, etc.) |
| `apps/client/src/lib/androidSetup.ts` | Native init: status bar, splash hide, back button, app lifecycle |
| `apps/client/ANDROID_BUILD_GUIDE.md` | Step-by-step APK build + Play Store guide |

### Build commands (from `apps/client`)
```bash
pnpm build:android    # Vite build with VITE_CAPACITOR=true (no service worker)
pnpm cap:sync         # Build + sync web assets into Android project
pnpm cap:open         # Open android/ in Android Studio
pnpm cap:icons        # Regenerate all mipmap icon PNGs from SVG
pnpm cap:splash       # Regenerate splash screen PNG from SVG
```

### Native plugins installed
`@capacitor/app`, `@capacitor/haptics`, `@capacitor/keyboard`, `@capacitor/local-notifications`, `@capacitor/network`, `@capacitor/push-notifications`, `@capacitor/splash-screen`, `@capacitor/status-bar`

### Notes
- `haptics.ts` uses native `Haptics` plugin on Android; falls back to `navigator.vibrate` on web.
- `push.ts` uses FCM (`@capacitor/push-notifications`) on Android; Web Push on all other platforms.
- `biometric.ts` uses WebAuthn which works natively inside Capacitor's WebView on Android.
- FCM push requires `google-services.json` — add the Android app to Firebase project `chats-19114` and place the file at `android/app/google-services.json`.
- The PWA service worker is disabled when `VITE_CAPACITOR=true` (SWs don't work in Capacitor WebView).
- APK building requires Android Studio on the developer's local machine (Replit has no Android SDK).
